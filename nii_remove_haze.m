...
%use Otsu's method to find binary threshold
mask = otsuSub(img(:),4);
mask(mask < 2) = 0;
mask(mask > 1) = 1;
mask = reshape(mask, size(img));
%find voxelwise connectedness
lbl = spm_bwlabel(mask, 26);
%identify biggest connected region
maxVox = 0;
maxIndx = 1;
for i = 1: max(lbl(:))
    vox = sum(lbl(:)==i);
...
fprintf('Found object with %d voxels\n', maxVox);
mask(lbl ~= maxIndx) = 0;
mask(mask ~= 0) = 1;
